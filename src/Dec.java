    ...
    public static Element dec(int d, Element[] P_A, Element[] S_A, Element[] P_B, Element[] S_B, PARS pars, Element[] D_i, Element[] d_i, Element[] D_i_prime, Element[] d_i_prime, Element C_0, Element C_1, Element C_2, Element[] C_1_i, Element[] C_2_i, Element[] C_3_i, Element[] C_4_i, Element[] C_5_i){
        ...
        if (W_A_prime.length >= d && W_B_prime.length >= d){
            ...
            for (int i = 0; i < W_A.length; i++) {
                K_l_prime = K_l_prime.mul(pars.getPairing().pairing(D_i_prime[i].duplicate(), C_1.duplicate()).duplicate().mul(pars.getPairing().pairing(Utils.H(W_A[i], pars).duplicate(), C_3_i[i].duplicate()).duplicate().mul(
                        pars.getPairing().pairing(pars.getPairing().getG1().newElementFromBytes(Utils.byteMergerAll(C_0.toBytes(), C_1.toBytes(), C_2.toBytes(), C_1_i[i].toBytes(), C_2_i[i].toBytes(), C_3_i[i].toBytes(), C_4_i[i].toBytes())).duplicate(),C_4_i[i].duplicate())).duplicate().mul(C_2.duplicate())).div(pars.getPairing().pairing(d_i_prime[i].duplicate(), C_2_i[i].duplicate()).duplicate().mul(pars.getPairing().pairing(C_5_i[i].duplicate(), pars.get_g().duplicate()).duplicate())).powZn(Utils.delta(pars.getZp().newZeroElement(), W_A[i], W_A, pars))).getImmutable();
            }
